...
export const BodyGenerator = {
    TransporterHelp: function(room: Room){
        // 后续需要调整成可以带WORK的
    ...
    Transporter: function(room: Room){
        // 50/3=16.6666666667
    ...
    EnergyHarvester: function(room: Room, node_index: number){
        // 上限
        // 1: 300
        // 2: 550
        // 3: 800
    ...
    MineralHarvester: function(room: Room){
        // 50/5=10
        // 4 WORK + 1 MOVE
    ...
    OutsideDefender: function(room: Room){
        // 4级以上才能有外矿
    ...
    OutsideReserver: function(room: Room){
        return [CLAIM, CLAIM, MOVE, MOVE]
    },
    OutsideEnergyHarvester: function(room: Room){
        // 外矿采集者，需要保证到岗时间
        // 4级以上才有外矿
        // 4: 1300
        // 5: 1800
    ...
    // 外矿运输者
    // 目前先一样，后面会加WORK
    OutsideTransporter: function(room: Room){
        // 50/3=16.6666666667
        const group_amount = Math.min(Math.floor(room.energyCapacityAvailable / 150), 16) ;
        const body: BodyPartConstant[] = []
        for (let i=0;i<group_amount;i++){
