const BODY_CONFIG = {
  ...
  '采集者R4': [WORK, WORK, WORK, WORK, WORK, WORK, WORK, WORK, WORK, WORK,
        WORK, CARRY, MOVE, MOVE, MOVE],  // WORK*11 + CARRY*1 + MOVE*3 = 1100
}
...
// body body组成数组
// amount 至少维持的数量
// ahead 如果有生命时间少于ahead的，提前<ahead>值tick，生成下一个，以免断档
const ACTIVE_ROLE_CONFIG = new Map([
  // ['Gather-B', { body: BODY_CONFIG['WORKER_R3B'], amount: 1, ahead: 80, memory: {role:'Gather', node:1} }],  // W35N57 下方矿点采集

  // 收集者
  ['GA-B', { basename:'', body: BODY_CONFIG['采集者R4'], amount: 1, ahead: 80, memory: {r:'采集', node:1} }],  // W35N57 下方矿点采集
  ['TR', { body: BODY_CONFIG['CARRYER_R3'], amount: 1, ahead: 80, memory: {r:'运输', e: 0} }],   // W35N57 将各个节点额外的能量搬运到Storage
  ['GA-A', { body: BODY_CONFIG['采集者R4'], amount: 1, ahead: 80, memory: {r:'采集', node:0} }],  // W35N57 下方矿点采集
  // ['Gather-B', { body: BODY_CONFIG['采集者R4'], amount: 1, ahead: 80, memory: {role:'Gather', node:1} }],  // W35N57 下方矿点采集
  // ['Gather-A', { body: BODY_CONFIG['采集者R4'], amount: 1, ahead: 80, memory: {role:'Gather', node:0} }],  // W35N57 上方矿点采集
  // ['Carryer-B', { body: BODY_CONFIG['CARRYER_R2'], amount: 1, ahead: 50, memory: {role:'Carryer', route: 0} }],   // W35N57 下方矿点搬运，扩展→母巢
  ...
  ['Builder-R', { body: BODY_CONFIG['BUILDER_R3'], amount: 1, memory: {r:'建造', model:'R'} }],  // 修理优先
  ['Upgrader-B', { body: BODY_CONFIG['BUILDER_R3'], amount: 3, memory: {r:'升级'} }],
  ...
  check: function() {
    ...
    for (const name in Game.creeps){
      ...
      if (m != 'unknown'){
        ...
        if (!(config && config.ahead && creep.ticksToLive! <= config.ahead)){
    ...
    for (const [basename,config] of ACTIVE_ROLE_CONFIG){
      ...
      const max = config.ahead ? config.amount + 1 : config.amount
